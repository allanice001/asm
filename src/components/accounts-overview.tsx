...
import { Building, CheckCircle, AlertCircle, Clock, FolderTree, List } from "lucide-react"
...
export function AccountsOverview() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (accountsData.lastSynced) {
        setLastSynced(new Date(accountsData.lastSynced))
      }
    } catch (error) {
  ...
  const getAccountStatus = (account: Account) => {
    if (!account.permissionSetAssignments || account.permissionSetAssignments.length === 0) {
      return "pending"
    }

    if (account.permissionSetAssignments.some((a) => a.status === "out-of-sync")) {
  ...
  const compliantCount = filteredAccounts.filter((a) => getAccountStatus(a) === "compliant").length
  const compliancePercentage =
    filteredAccounts.length > 0 ? Math.round((compliantCount / filteredAccounts.length) * 100) : 0
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <AccountSync onSyncComplete={fetchData} lastSynced={lastSynced} />
      ...
      <CardContent>
        {loading ? (
          ...
          <>
            <AccountSearch onSearch={handleSearch} onClear={handleClearSearch} query={searchQuery} />
                      ...
                      return (
                        <div key={account.id} className="flex items-center justify-between border p-3 rounded-md">
                          <div>
                            ...
                            {account.ouName && (
                              <div className="text-xs text-muted-foreground mt-1">OU: {account.ouName}</div>
                            )}
                            <div className="text-xs text-muted-foreground mt-1">
                              {account.permissionSetAssignments?.length || 0} permission sets assigned
                            </div>
                          </div>
                          <Badge
                            variant={
                          ...
                          >
                            {status === "compliant" && <CheckCircle className="h-3 w-3" />}
              ...
              <TabsContent value="tree" className="m-0">
                <div className="border rounded-md p-4">
                  {ouStructure.length === 0 ? (
                    <div className="text-center py-6 text-muted-foreground">No organizational units found.</div>
                  ) : (
                    <OUTree
                      data={ouStructure}
